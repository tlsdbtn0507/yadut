...
SKILLS=(
    ...
    "cesp:sounds:Manage CESP sound packs"
)
...
# 1c. Install CESP Module
echo "🎵 Installing CESP sound support..."
mkdir -p "$GEMINI_SKILLS_BASE_DIR/cesp"
cp -r "$(dirname "$0")/src/cesp" "$GEMINI_SKILLS_BASE_DIR/"
...
for entry in "${SKILLS[@]}"; do
    ...
    # Special handling for sounds command which might not have a SKILL.md in cache
    if [ "$cmd_name" == "sounds" ]; then
        cat > "$toml_file" <<'EOF'
description = "Manage CESP sound packs"
prompt = """
I am use the CESP Sound Pack Manager.

Available commands:
- `PYTHONPATH=.gemini/.superpowers/skills python3 -m cesp.cli list` - List installed packs
- `PYTHONPATH=.gemini/.superpowers/skills python3 -m cesp.cli search` - Search registry
- `PYTHONPATH=.gemini/.superpowers/skills python3 -m cesp.cli install <name>` - Install a pack
- `PYTHONPATH=.gemini/.superpowers/skills python3 -m cesp.cli use <name>` - Set active pack
- `PYTHONPATH=.gemini/.superpowers/skills python3 -m cesp.cli volume <0.0-1.0>` - Set volume

Task: {{args}}
"""
EOF
        echo "   ✓ /sounds -> CESP Manager"
        ((++count))
        continue
    fi
    ...
    if [ -f "$GEMINI_SKILLS_BASE_DIR/$skill_dir/SKILL.md" ]; then
        # We use Gemini CLIs native @{path} syntax with a pointer to skills.
...
## AUDIO FEEDBACK (CESP)
If a sound pack is installed, you should play sounds for these events using `PYTHONPATH=.gemini/.superpowers/skills python3 -m cesp.cli emit <category>`:
- **task.acknowledge**: When you accept a task and start working.
- **task.complete**: When you successfully finish a task or the entire plan.
- **task.error**: When a test fails or you hit an error.
- **input.required**: When you need to ask the user a question.
...
- Use `/sounds` (cesp) to manage sound packs.
...
You have access to the following specialized skills. To activate a skill and receive its detailed instructions, you can call the `activate_skill` tool with the skills name.

<available_skills>
  <skill>
    <name>skill-creator</name>
    <description>Guide for creating effective skills. This skill should be used when users want to create a new skill (or update an existing skill) that extends Gemini CLIs capabilities with specialized knowledge, workflows, or tool integrations.</description>
